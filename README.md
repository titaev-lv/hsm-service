...
**Для компаний, обрабатывающих платежные карты, PCI DSS - обязательный стандарт**. HSM Service покрывает критические требования:
...
#### 💰 Экономия на PCI DSS аудите

**Без HSM Service**: аудиторы проверяют каждый микросервис отдельно
- 🕐 50+ сервисов × 2 часа аудита = 100+ часов
- 💸 Стоимость: $200-400/час × 100 = $20,000-40,000

**С HSM Service**: централизованный аудит ключей
- ✅ Один компонент для проверки
- ✅ Автоматические compliance отчеты
- ⏱️ Экономия: ~80% времени аудита
- 💵 Экономия: ~$15,000-30,000 на аудит
...
- **Собственный Certificate Authority (CA)** - ключи и сертификат для подписи клиентских/серверных сертификатов

### Подготовка PKI инфраструктуры

#### 1. Создание собственного CA (если его нет)

**⚠️ Важно**: Для работы mTLS необходим собственный Certificate Authority.

```bash
# Создать приватный ключ CA (защищенный паролем)
...
**Куда поместить CA файлы**:
```bash
# Скопировать CA ключ и сертификат в директорию проекта
cp ca.key /path/to/hsm-service/pki/ca/
cp ca.crt /path/to/hsm-service/pki/ca/

# Установить правильные права доступа
chmod 600 /path/to/hsm-service/pki/ca/ca.key
chmod 644 /path/to/hsm-service/pki/ca/ca.crt
```

**Структура pki/ca/ должна выглядеть так**:
```
pki/ca/
├── ca.crt          # Сертификат CA (публичный)
├── ca.key          # Приватный ключ CA (защищен паролем)
└── ca.srl          # Serial number file (создается автоматически)
```
...
#### 2. Генерация сертификатов для HSM Service

После того как CA готов:

```bash
# Сгенерировать серверный сертификат для HSM Service
./pki/scripts/issue-server-cert.sh hsm-service.local

# Сгенерировать клиентский сертификат для Trading сервиса
./pki/scripts/issue-client-cert.sh hsm-trading-client-1 Trading

# Сгенерировать клиентский сертификат для 2FA сервиса
./pki/scripts/issue-client-cert.sh hsm-2fa-client-1 2FA
```
...
# 2. Подготовить CA (см. выше)
# Убедитесь, что pki/ca/ca.key и pki/ca/ca.crt на месте

# 3. Сгенерировать PKI инфраструктуру
./pki/scripts/issue-server-cert.sh hsm-service.local
./pki/scripts/issue-client-cert.sh hsm-trading-client-1 Trading

# 4. Создать metadata.yaml из шаблона
cp metadata.yaml.example metadata.yaml

# 5. Запустить сервис
docker-compose up -d

# 6. Инициализировать KEK (первый раз)
