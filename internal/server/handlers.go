...
// 6. Get active key for context
keyLabels := hsmCtx.GetKeyLabels()
if len(keyLabels) == 0 {
slog.Error("no KEKs available in HSM")
respondError(w, http.StatusInternalServerError, "no keys available")
return
}
// Use first available key (TODO: implement key selection by context)
keyID := keyLabels[0]
